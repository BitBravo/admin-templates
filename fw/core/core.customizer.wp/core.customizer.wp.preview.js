...
( function( $ ) {
	...
	if (!cars4rent_previewer_vars['need_refresh']) {
		var $style = $('#cars4rent-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="cars4rent-customizer-css"></'+'style'+'>' ).find('#cars4rent-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			cars4rent_set_cookie(cars4rent_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
