/* global cars4rent_color_schemes, cars4rent_dependencies, Color */
...
( function( api ) {
	...
	if (!cars4rent_customizer_vars['need_refresh']) {
		for (var i in cars4rent_color_schemes) {
			cssTemplate[i] = wp.template( 'cars4rent-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'cars4rent-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (cars4rent_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+cars4rent_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			cars4rent_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		cars4rent_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		cars4rent_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		cars4rent_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function cars4rent_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in cars4rent_dependencies) {
				if (fld == id) {
					depend = cars4rent_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('cars4rent_options_no_use');
				} else {
					jQuery(this).hide().addClass('cars4rent_options_no_use');
				}
			}
		});
	}
	...
	function cars4rent_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			cars4rent_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in cars4rent_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					cars4rent_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							cars4rent_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							cars4rent_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in cars4rent_fonts) {
					for (var prop in cars4rent_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							cars4rent_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) cars4rent_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function cars4rent_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		cars4rent_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(cars4rent_serialize(cars4rent_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function cars4rent_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in cars4rent_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( cars4rent_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', cars4rent_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', cars4rent_color_schemes[value][opt] );
		}
		updateCSS = true;
		cars4rent_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function cars4rent_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		cars4rent_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function cars4rent_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || cars4rent_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in cars4rent_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in cars4rent_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = cars4rent_color_schemes[scheme][i];
			}
			
			// Make theme specific colors and tints
			colors = cars4rent_customizer_wp_add_theme_colors(colors);
		...
		for (var tag in cars4rent_fonts) {
			...
			fonts[tag+'_font-size'] = font['font-size'] && font['font-size']!='inherit'
												? 'font-size:' + cars4rent_prepare_css_value(font['font-size']) + ';'
												: '';
			fonts[tag+'_line-height'] = font['line-height'] && font['line-height']!='inherit'
												? 'line-height:' + cars4rent_prepare_css_value(font['line-height']) + ';'
												: '';
			fonts[tag+'_line-height_value'] = font['line-height']	// && font['line-height']!='inherit'
												? cars4rent_prepare_css_value(font['line-height'])
												: 'inherit';
			...
			fonts[tag+'_margin-top'] = font['margin-top'] && font['margin-top']!='inherit'
												? "margin-top:" + cars4rent_prepare_css_value(font['margin-top']) + ";"
												: '';
			fonts[tag+'_margin-top_value'] = font['margin-top']	// && font['margin-top']!='inherit'
												? cars4rent_prepare_css_value(font['margin-top'])
												: 'inherit';
			fonts[tag+'_margin-bottom'] = font['margin-bottom'] && font['margin-bottom']!='inherit'
												? "margin-bottom:" + cars4rent_prepare_css_value(font['margin-bottom']) + ";"
												: '';
			fonts[tag+'_margin-bottom_value'] = font['margin-bottom']	// && font['margin-bottom']!='inherit'
												? cars4rent_prepare_css_value(font['margin-bottom'])
												: 'inherit';
		}
			
		// Make theme specific colors and tints
		if (window.cars4rent_customizer_wp_add_theme_fonts) fonts = cars4rent_customizer_wp_add_theme_fonts(fonts);
	...
	function cars4rent_customizer_wp_add_theme_colors(colors) {
	...
	// Attention! Don't forget setup custom colors also in the theme.styles.no_less.php
	function cars4rent_customizer_wp_add_theme_fonts(fonts) {
		fonts['logo_margin-top_0_75']	= cars4rent_multiply_css_value( fonts['logo_margin-top_value'], 0.75 );
		fonts['logo_margin-bottom_0_5']	= cars4rent_multiply_css_value( fonts['logo_margin-bottom_value'], 0.5 );
		fonts['menu_margin-top_1']		= fonts['menu_margin-top_value'];
		fonts['menu_margin-top_0_3']	= cars4rent_multiply_css_value( fonts['menu_margin-top_value'], 0.3 );
		fonts['menu_margin-top_0_6']	= cars4rent_multiply_css_value( fonts['menu_margin-top_value'], 0.6 );
		fonts['menu_margin-top_0_65']	= cars4rent_multiply_css_value( fonts['menu_margin-top_value'], 0.65 );
		fonts['menu_margin-top_0_7']	= cars4rent_multiply_css_value( fonts['menu_margin-top_value'], 0.7 );
		fonts['menu_margin-top_0_8']	= cars4rent_multiply_css_value( fonts['menu_margin-top_value'], 0.8 );
		fonts['menu_margin-bottom_1']	= fonts['menu_margin-bottom_value'];
		fonts['menu_margin-bottom_0_5']	= cars4rent_multiply_css_value( fonts['menu_margin-bottom_value'], 0.5 );
		fonts['menu_margin-bottom_0_6']	= cars4rent_multiply_css_value( fonts['menu_margin-bottom_value'], 0.6 );
		fonts['menu_height_1']			= cars4rent_summ_css_value( cars4rent_summ_css_value( fonts['menu_margin-top_value'],  fonts['menu_margin-bottom_value'] ), fonts['menu_line-height_value'] );
