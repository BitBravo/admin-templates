...
function cars4rent_ready_actions() {
	...
	cars4rent_init_sfmenu('.menu_main_nav_area ul#menu_main, ul#menu_user, ul#menu_side, body:not(.woocommerce) .widget_area:not(.footer_wrap) .widget_product_categories ul.product-categories');
	...
	jQuery('.woocommerce,.woocommerce-page').on('click', '.mode_buttons a', function(e) {
		...
		jQuery.cookie('cars4rent_shop_mode', mode, {expires: 365, path: '/'});
...
function cars4rent_scroll_actions() {
	...
	// Call theme specific action (if exists)
    //----------------------------------------------
	if (window.cars4rent_theme_scroll_actions) cars4rent_theme_scroll_actions();
...
function cars4rent_resize_actions() {
	...
	// Call theme specific action (if exists)
    //----------------------------------------------
	if (window.cars4rent_theme_resize_actions) cars4rent_theme_resize_actions();
	...
	if (sb.length > 0 && cat_menu.length > 0) {
		if (sb.width() == sb.parents('.content_wrap').width()) {
			...
			if (!cat_menu.hasClass('inited')) {
				...
				cars4rent_init_sfmenu('body:not(.woocommerce) .widget_area:not(.footer_wrap) .widget_product_categories ul.product-categories');
			}
		}
	}
}
