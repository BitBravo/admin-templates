jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(CARS4RENT_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == CARS4RENT_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(CARS4RENT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				cars4rent_message_success('', CARS4RENT_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(CARS4RENT_STORAGE['strings']['editor_caption_close']);
			} else {
				cars4rent_message_warning(rez.error, CARS4RENT_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		cars4rent_message_confirm(CARS4RENT_STORAGE['strings']['editor_delete_post'], CARS4RENT_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(CARS4RENT_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					cars4rent_message_success('', CARS4RENT_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = CARS4RENT_STORAGE['site_url'];
						}, 1000);
				} else {
					cars4rent_message_warning(rez.error, CARS4RENT_STORAGE['strings']['editor_delete_error']);
				}
			});
