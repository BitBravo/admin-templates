...
if ( !function_exists( 'cars4rent_template_news_excerpt_theme_setup' ) ) {
	add_action( 'cars4rent_action_before_init_theme', 'cars4rent_template_news_excerpt_theme_setup', 1 );
	function cars4rent_template_news_excerpt_theme_setup() {
		cars4rent_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'cars4rent'),
...
// Template output
if ( !function_exists( 'cars4rent_template_news_excerpt_output' ) ) {
	function cars4rent_template_news_excerpt_output($post_options, $post_data) {
				...
				if ( !in_array($post_data['post_format'], array('link', 'aside', 'status', 'quote')) ) {
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<h5 class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php cars4rent_show_layout($post_data['post_title']); ?></a></h5>
							<?php
						} else {
							?>
							<h5 class="post_title entry-title"><?php cars4rent_show_layout($post_data['post_title']); ?></h5>
							<?php
						}
					}
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(cars4rent_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: cars4rent_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
