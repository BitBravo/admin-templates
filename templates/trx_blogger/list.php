...
if ( !function_exists( 'cars4rent_template_list_theme_setup' ) ) {
	add_action( 'cars4rent_action_before_init_theme', 'cars4rent_template_list_theme_setup', 1 );
	function cars4rent_template_list_theme_setup() {
		cars4rent_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'cars4rent')
			));
	}
}

// Template output
if ( !function_exists( 'cars4rent_template_list_output' ) ) {
	function cars4rent_template_list_output($post_options, $post_data) {
