...
function cars4rent_theme_customizer(option, val) {
	"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = CARS4RENT_STORAGE['co_add_params']!=undefined ? CARS4RENT_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#cars4rent-customizer-styles-'+option).length > 0 ? jQuery('#cars4rent-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="cars4rent-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
